    ...
    <div className='carrousel-container'>
      <Draggable ref={draggableContainer} draggable={draggable}>
        {images.map((image, index) => (
          index === 0 ?
            <CarrouselCard ref={(el) => setItemRef(el, index)} key={index} carrouselOpen={open} img={image} title={title} subtitle={subtitle} onOpen={onOpen} onClose={onCloseGallery}/> :
            <CarrouselCard ref={(el) => setItemRef(el, index)} key={index} carrouselOpen={open} img={image} onClose={onCloseGallery}/>
        ))}
